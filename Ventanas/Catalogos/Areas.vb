Public Class Areas
    ...
    Private Sub cargarListado()
        Try
            ...
            dts = _qry.SelToGridWhere("*", "[sga.cat_Areas]", "area_id>0")
            If dts.Tables(0).Rows.Count > 0 Then
                For Each row As DataRow In dts.Tables(0).Rows
                    dgvCatalogo.Rows.Add(row("area_id"), row("area_Desc"))
                Next
            End If
    ...
    Private Sub guardarCambios()
        Try
            _qry.ActulizarGral("[sga.cat_Areas]", "area_Desc='" & txtDesc.Text.Replace("'", "''") & "'", "area_id=" & txtId.Text)
        Catch ex As Exception
    ...
    Private Sub InsertarNuevo()
        Try
            _qry.InsertarDatos("area_Desc", "'" & txtDesc.Text.Replace("'", "''") & "'", "[sga.cat_Areas]")
        Catch ex As Exception
            MsgBox("Error al crear nueva Área. " & ex.Message)
        End Try
    End Sub
